...
<p style="font-size:12px;color:{{token.subTextColor}};text-align:center;margin:0 0 24px;">
  ...
  <table style="width:100%;border-collapse:collapse;font-size:13px;background:#fff;border:2px solid {{token.borderColor}};border-radius:8px;overflow:hidden;">
    ...
    <tbody>{{rows}}</tbody>
      ...
      orderedListItem: `<section style="margin-bottom:12px;"><span style="display:inline-block;width:24px;height:24px;line-height:24px;text-align:center;font-size:11px;font-weight:700;color:#fff;background:{{token.primary}};border:2px solid {{token.primaryDark}};border-radius:4px;margin-right:10px;vertical-align:top;box-sizing:border-box;"><span leaf="">{{index}}</span></span><span style="display:inline-block;font-size:14px;color:{{token.textColor}};line-height:1.8;vertical-align:top;"><span leaf="">{{content}}</span></span></section>`,
